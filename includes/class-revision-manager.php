...
class AI_SEO_GEO_Revision_Manager {
	...
	public function apply_selected_changes( $job_id, $post_id, $selected_fields ) {
		...
		$detected_editor  = isset( $result_json['detected_editor'] ) ? sanitize_text_field( $result_json['detected_editor'] ) : '';
		$output_format    = isset( $result_json['output_format'] ) ? sanitize_text_field( $result_json['output_format'] ) : '';
		$safe_to_apply    = ! isset( $result_json['safe_to_apply_content'] ) || (bool) $result_json['safe_to_apply_content'];
		...
		if ( in_array( 'content', $selected_fields, true ) ) {
			$forbidden = (array) ( $result_json['forbidden_phrases_found'] ?? array() );
			$strong = array( 'No.1', '100% guaranteed', 'lowest price', 'world-leading', 'best manufacturer' );
			if ( count( array_intersect( $strong, $forbidden ) ) > 0 ) {
				$blocked_fields[] = 'content';
				return array( 'success' => false, 'message' => __( 'Strong risk phrases found; content apply blocked.', 'ai-seo-geo-optimizer' ) );
			}
			if ( ! $safe_to_apply || 'plain_text_brief' === $output_format ) {
				return array( 'success' => false, 'message' => __( 'Current format is not safe for direct content apply.', 'ai-seo-geo-optimizer' ) );
			}
			if ( 'elementor' === $detected_editor ) {
				return array( 'success' => false, 'message' => __( 'Elementor content apply is blocked. Please copy sections manually.', 'ai-seo-geo-optimizer' ) );
			}
			$optimized_content = isset( $result_json['optimized_content'] ) ? trim( (string) $result_json['optimized_content'] ) : '';
			if ( 'gutenberg' === $detected_editor ) {
				if ( ! empty( $result_json['gutenberg_content'] ) ) {
					$optimized_content = trim( (string) $result_json['gutenberg_content'] );
				} else {
					return array( 'success' => false, 'message' => __( 'Gutenberg content missing. Please regenerate with block markup.', 'ai-seo-geo-optimizer' ) );
				}
			}
			...
			if ( 'product' === $post_type ) {
				$short = isset( $result_json['product_short_description'] ) ? trim( (string) $result_json['product_short_description'] ) : '';
				$long  = isset( $result_json['product_long_description'] ) ? trim( (string) $result_json['product_long_description'] ) : $optimized_content;
				if ( '' !== $short ) {
					$applied_fields[] = 'product_short_description';
					$post_data_update['post_excerpt'] = sanitize_textarea_field( $short );
				}
				if ( '' !== $long ) {
					$applied_fields[] = 'product_long_description';
					$post_data_update['post_content'] = wp_kses_post( $long );
				}
			} else {
				$post_data_update['post_content'] = wp_kses_post( $optimized_content );
				$applied_fields[] = 'content';
			}
		}
