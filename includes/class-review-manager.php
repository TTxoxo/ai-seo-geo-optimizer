...
class AI_SEO_GEO_Review_Manager {
	...
	/**
	 * Ensures readability fields exist in AI result.
	 *
	 * @param array $result Result payload.
	 * @return array
	 */
	public function ensure_readability_fields( $result ) {
		$defaults = array(
			'human_readability_score'   => 0,
			'ai_style_risk'             => 'medium',
			'forbidden_phrases_found'   => array(),
			'generic_marketing_phrases' => array(),
			'overused_transitions'      => array(),
			'keyword_stuffing_warnings' => array(),
			'recommended_human_edits'   => array(),
		);

		return array_merge( $defaults, is_array( $result ) ? $result : array() );
	}
